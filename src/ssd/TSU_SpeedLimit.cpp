...
namespace SSD_Components
{
		...
		max_user_write_arrival_count(channel_count* chip_count_per_channel* die_count_per_chip* plane_count_per_die)
	{
		...
		remain_in_queue_count = new unsigned long** [channel_count];
		...
		for (unsigned int channel_id = 0; channel_id < channel_count; channel_id++)
		{
			...
			remain_in_queue_count[channel_id] = new unsigned long* [chip_count_per_channel];
			...
			for (unsigned int chip_id = 0; chip_id < chip_count_per_channel; chip_id++)
			{
				...
				remain_in_queue_count[channel_id][chip_id] = new unsigned long[stream_count];
				...
				for (unsigned int stream_id = 0; stream_id < stream_count; ++stream_id)
				{
					...
					remain_in_queue_count[channel_id][chip_id][stream_id] = 0;
	...
	TSU_SpeedLimit::~TSU_SpeedLimit()
	{
		...
		for (stream_id_type stream_id = 0; stream_id < stream_count; ++stream_id)
		{
			...
			std::cout << "shared\t" << shared_total_time[stream_id] / total_count[stream_id] / 1000
				...
				<< "\nsum slowdown\t" << (double)shared_total_time[stream_id] / alone_total_time[stream_id]
		...
		for (unsigned int channel_id = 0; channel_id < channel_count; channel_id++)
		{
			...
			for (unsigned int chip_id = 0; chip_id < chip_no_per_channel; ++chip_id)
			{
				...
				delete[] remain_in_queue_count[channel_id][chip_id];

			}
			delete[] serviced_writes_since_last_GC[channel_id];

			delete[] remain_in_queue_count[channel_id];
	...
	void TSU_SpeedLimit::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		{
			alone_total_time[transaction->Stream_id] += 0.5 * buffer_time + 0.23 * queue_time + backend_time;
		}
		else if (transaction->Type == Transaction_Type::WRITE)
		{
			if (buffer_time > queue_time)
			{
				alone_total_time[transaction->Stream_id] += 0.35 * buffer_time + backend_time - 0.05 * queue_time;
			}
			else
			{
				alone_total_time[transaction->Stream_id] += 0.41 * queue_time + backend_time - 0.12 * buffer_time;
			}
		}*/
			...
			it != transaction_receive_slots.end(); it++)
		{
			...
			case Transaction_Type::READ:
				...
				case Transaction_Source_Type::USERIO:
					...
					estimate_alone_time(*it, remain_in_queue_count[(*it)->Address.ChannelID][(*it)->Address.ChipID][(*it)->Stream_id]);
					//estimate_alone_time(*it, UserReadTRBuffer, &UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]);
					remain_in_queue_count[(*it)->Address.ChannelID][(*it)->Address.ChipID][(*it)->Stream_id]++;
					if (stream_count == 1)
					{
			...
			case Transaction_Type::WRITE:
				...
				case Transaction_Source_Type::USERIO:
					(*it)->buffer_time = Simulator->Time();
					estimate_alone_time(*it, remain_in_queue_count[(*it)->Address.ChannelID][(*it)->Address.ChipID][(*it)->Stream_id]);
					//estimate_alone_time(*it, UserWriteTRBuffer, &UserWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]);
					remain_in_queue_count[(*it)->Address.ChannelID][(*it)->Address.ChipID][(*it)->Stream_id]++;
					if (stream_count == 1)
					{
		...
		unsigned int* UserTRCount, unsigned int* user_limit_speed, std::vector<stream_id_type>& user_idx)
	{
		...
		for (unsigned int id = 0; id < stream_count; ++id)
		{
			...
			while (!(UserTRBuffer[stream_id].empty()) && UserTRCount[stream_id] < user_limit_speed[stream_id])
			{
				...
				estimate_shared_time(it, &UserTRQueue[it->Address.ChannelID][it->Address.ChipID]);
	...
	void TSU_SpeedLimit::estimate_alone_time(NVM_Transaction_Flash* transaction, Flash_Transaction_Queue* buffer, Flash_Transaction_Queue* queue)
	{
		...
		/*if (buffer->empty())
		{
			...
			if (queue->size())
			{
				auto itr_it = std::prev(queue->end());
				while (itr_it != queue->begin() && (*itr_it)->Stream_id != transaction->Stream_id)
				{
					--itr_it;
				}
				if ((*itr_it)->Stream_id == transaction->Stream_id && (*itr_it)->alone_time + (*itr_it)->Issue_time > Simulator->Time())
				{
					miss_last = false;
					waiting_last_time = (*itr_it)->alone_time + (*itr_it)->Issue_time - Simulator->Time();
				}
			}
			if (miss_last)
			{
		...
		{
			NVM_Transaction_Flash* tr = buffer->back();
			if (tr->alone_time + tr->Issue_time > Simulator->Time())
			{
				waiting_last_time = tr->alone_time + tr->Issue_time - Simulator->Time();
			}
		}*/
		NVM_Transaction_Flash* chip_tr = _NVMController->Is_chip_busy_with_stream(transaction);
		if (chip_tr && _NVMController->Expected_finish_time(chip_tr) > Simulator->Time())
		...
		if (transaction->Stream_id == 0)
			waiting_last_time *= 0.5;
		else
			waiting_last_time *= 0.5;
		if (buffer->size() && transaction->Stream_id == 1)
		{
			waiting_last_time += buffer->size() / 32
		...
		transaction->alone_time = chip_busy_time + waiting_last_time
			+ _NVMController->Expected_command_time(transaction) + _NVMController->Expected_transfer_time(transaction);
		/*std::cout << "transaction\t" << chip_busy_time << "\t" << waiting_last_time << "\t"
			<< _NVMController->Expected_command_time(transaction) + _NVMController->Expected_transfer_time(transaction) << "\n";*/
	}
	...
	void TSU_SpeedLimit::estimate_shared_time(NVM_Transaction_Flash* transaction, Flash_Transaction_Queue* queue)
	{
		sim_time_type chip_busy_time = 0, waiting_last_time = 0;
		//if (queue->empty())
		{
		...
		//else
		{
			/*NVM_Transaction_Flash* tr = queue->back();
			if (tr->Issue_time + tr->shared_time > Simulator->Time())
			{
				waiting_last_time = tr->Issue_time + tr->shared_time - Simulator->Time();
			}*/
			if (queue->size())
			{
				auto itr_it = queue->end();
		...
		transaction->shared_time = chip_busy_time + waiting_last_time + (Simulator->Time() - transaction->buffer_time)
			+ _NVMController->Expected_transfer_time(transaction) + _NVMController->Expected_command_time(transaction);
	}
					...
					if ((*it)->Address.DieID == die_id && !(plane_vector & 1 << (*it)->Address.PlaneID))
					{
						...
						if ((*it)->Address.PageID == page_id)
						{
								...
								|| (*it)->Source == Transaction_Source_Type::MAPPING)
							{
								...
								remain_in_queue_count[chip->ChannelID][chip->ChipID][(*it)->Stream_id]--;
							}
